...
Msg *mysql_fetch_msg()
{
    ...
    if (res == NULL) {
        ...
        if (mysql_num_rows(res) >= 1) {
            ...
            msg->sms.msgdata  = octstr_null_create(row[26]);
...
void mysql_save_msg(Msg *msg, Octstr *momt)
{
    ...
    sql = octstr_format(SQLBOX_MYSQL_INSERT_QUERY, sqlbox_logtable, st_str(momt), st_str(msg->sms.sender),
        ...
        st_str(msg->sms.boxc_id), st_str(msg->sms.binfo), st_str(msg->sms.msgdata), st_num(msg->sms.priority), st_str(msg->sms.foreign_id));
    sql_update(sql);
    while (stuffcount > 0) {
